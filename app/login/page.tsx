...
export default function LoginPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!data?.token) {
        toast.error(t("login.toast.loginFailed"), { description: t("login.toast.noToken") });
        return;
      }
      ...
      const msg =
        axios.isAxiosError(err) && err.response?.data && typeof err.response.data === "object"
          ? String((err.response.data as { error?: string }).error ?? t("common.requestFailed"))
          : t("login.toast.requestFailed");
      toast.error(t("login.toast.loginFailed"), { description: msg });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="relative flex min-h-full flex-1 flex-col items-center justify-center bg-gradient-to-b from-zinc-100 to-zinc-200 px-4 py-16 dark:from-zinc-950 dark:to-black">
      ...
      <div className="mb-8 flex items-center gap-3 text-zinc-800 dark:text-zinc-100">
        ...
        <div>
          <h1 className="text-xl font-semibold tracking-tight">{t("meta.title")}</h1>
          <p className="text-sm text-zinc-600 dark:text-zinc-400">{t("login.brandSubtitle")}</p>
        </div>
      </div>
      ...
      >
        <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-50">{t("login.heading")}</h2>
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{t("login.hint")}</p>
        ...
        <p className="mt-6 text-center text-xs text-zinc-500">{t("login.policy")}</p>
      </form>
